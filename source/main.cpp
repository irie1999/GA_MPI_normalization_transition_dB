...
int main(int argc, char **argv){
    ...
    if(rank == 0){
        ...
        std::ofstream ofs("../data/score_graph" + std::to_string(Number_of_Generation) + ", " + std::to_string(Number_of_Individual) + ".dat");
        ...
        for(int i = 0; i < Number_of_Generation; i++){
            ofs_1   << "parameter_beta_1= " << MAX_parameter[0][i] << std::endl << " beta_2= " << MAX_parameter[1][i]  << std::endl <<  " h_prime_1= " << MAX_parameter[2][i] << std::endl
                    << " h_prime_2= " << MAX_parameter[3][i] << std::endl << std::endl;
        }
        ofs_1.close();
